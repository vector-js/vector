...
function sinusoidalWave(t) {
    return (a) => {
        return Math.max(a, Math.sin(a));
    };
}
export default function main(id) {
    ...
    // let delta = 46;
    let delta = 2;
    for (let i = 0; i < interactive.width; i += delta) {
        let theta = i / interactive.width;
        let rectangle = interactive.rectangle(i, 0, delta, 200);
        let r = trapezoidalWave(-2 / 6)(theta) * 255;
        let b = trapezoidalWave(2 / 6)(theta) * 255;
        let g = trapezoidalWave(0 / 6)(theta) * 255;
        rectangle.style.fill = `rgb( ${r}, ${g}, ${b})`;
        rectangle.style.stroke = `rgb( ${r}, ${g}, ${b})`;
        let rect1 = interactive.rectangle(i, 220, delta, 20);
        let rect2 = interactive.rectangle(i, 240, delta, 20);
        let rect3 = interactive.rectangle(i, 260, delta, 20);
        rect1.style.fill = `rgba( ${0}, ${0}, ${255}, ${b / 255})`;
        rect2.style.fill = `rgba( ${0}, ${255}, ${0}, ${g / 255})`;
        rect3.style.fill = `rgba( ${255}, ${0}, ${0}, ${r / 255})`;
    }
    let group = interactive.group();
    group.setAttribute('transform', 'translate(250, 500)');
    let circle = group.circle(0, 0, 200);
...
//# sourceMappingURL=visible-spectrum.js.map