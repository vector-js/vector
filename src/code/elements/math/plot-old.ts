...
export default class Plot extends SVG {
  ...
  */
  constructor( fn:(x:number) => number, options:PlotOptions ) {
    ...
    if( config.border ) {
      this._rect.style.border = '1px solid var(--main)';
    } else {
    ...
    // default values
    this.viewPort = this.clipGroup.svg(this._width, this._height);
    ...
    this.staticGroup.circle(0, 0, 3).fill = 'var(--main)';
  ...
  */
  export() : SVG {
    let result = new SVG(this._width, this._height);
