...
export default function main( id:string ) {
	...
	tree.update = function() {
	  ...
	  if( tree.exponent !== levelsValue || tree.base !== branchingValue ) {
		...
		katex.render(`\\begin{aligned}${tree.base}^${tree.exponent} &= ${tree.leaves} \\\\ \\log_${tree.base}(${tree.leaves}) &= ${tree.exponent} \\\\ \\sqrt[${tree.exponent}]{${tree.leaves}} &= ${tree.base} \\end{aligned}`, container, {
			displayMode: true,
		});
