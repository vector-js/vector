...
export class DegreesFigure {
	...
	constructor(id:string, options?:Options  ) {
		...
		let label = group.text( this.radius + this.margin/2, 0, '0, 360°');
		...
		for( let i = 0; i < config.divide; i ++ ) {
			...
			if( a != 0 ) {
				let label = group.text( x + (this.margin/2)*Math.cos(a), -y - (this.margin/2)*Math.sin(a), `${(a*360/TAU).toFixed(0)}`);
				label.tspan('°').classList.add('katex-variable');
				label.classList.add('katex-main', 'text-middle');
